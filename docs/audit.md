...
Only this macOS machine was exercised locally. The added Linux CI workflow has not yet been run on GitHub. Windows, Linux display setup, remote GPUs, external image providers and hardware printers are not certified here. Custom SCAD is trusted code with the current user's filesystem access. One server process owns an output directory; edits are serialized and old revisions are retained. No public server deployment, mesh repair, slicing, or physical printing is claimed.
